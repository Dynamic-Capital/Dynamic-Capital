...
DEFAULT_NODE_CONFIGS: tuple[Mapping[str, object], ...] = (
    {
        "node_id": "signal-intake",
        "type": "ingestion",
        "enabled": True,
        "interval_sec": 30,
        "dependencies": (),
        "outputs": ("raw_signals", "signals"),
        "metadata": {
            "description": "Normalise TradingView and discretionary alerts into the unified signal queue.",
            "sources": ("tradingview-webhook", "manual-desk"),
            "table": "signals",
        },
        "weight": 0.95,
    },
    {
        ...
        "enabled": True,
        "interval_sec": 21600,
        "dependencies": ("raw_signals",),
        "outputs": ("curated_signals", "fusion"),
        "metadata": {
            "description": "Blend discretionary desk review with automated scoring to curate actionable signals.",
            "source": "analyst_insights",
        },
        "weight": 0.35,
    },
    {
        "node_id": "signal-dispatch",
        "type": "processing",
        "enabled": True,
        "interval_sec": 45,
        "dependencies": ("curated_signals",),
        "outputs": ("signal_dispatches",),
        "metadata": {
            "description": "Claim pending signals and coordinate worker dispatch heartbeats.",
            "workers": ("mt5-bridge", "ton-router"),
            "table": "signal_dispatches",
        },
        "weight": 0.9,
    },
    {
        "node_id": "trade-execution-sync",
        "type": "ingestion",
        "enabled": True,
        "interval_sec": 60,
        "dependencies": ("signal_dispatches",),
        "outputs": ("trades",),
        "metadata": {
            "description": "Sync executed orders and positions from broker endpoints into the state store.",
            "brokers": ("exness", "mt5"),
            "tables": ("executions", "positions"),
        },
        "weight": 0.85,
    },
    {
        "node_id": "correlation-engine",
        "type": "processing",
        "enabled": True,
        "interval_sec": 300,
        "dependencies": ("trades",),
        ...
        "metadata": {
            "description": "Generate rolling correlation matrices across tracked symbols for hedging inputs.",
            "windows": ("1h", "4h", "1d"),
        },
        "weight": 0.7,
    },
    {
        "node_id": "risk-governance",
        "type": "processing",
        "enabled": True,
        "interval_sec": 900,
        "dependencies": ("correlations", "trades"),
        "outputs": ("risk_settings",),
        "metadata": {
            "description": "Translate correlation and policy thresholds into broker risk parameter adjustments.",
            "policy_docs": ("docs/risk-framework.md", "docs/dynamic-hedge-configuration.md"),
        },
        "weight": 0.6,
    },
    {
        ...
        "enabled": True,
        "interval_sec": 300,
        "dependencies": ("trades", "correlations", "risk_settings", "curated_signals"),
    ...
    {
        "node_id": "hedge-broadcast",
        "type": "community",
        "enabled": True,
        "interval_sec": 300,
        "dependencies": ("hedge_actions",),
        "outputs": ("community_alerts",),
        "metadata": {
            "description": "Broadcast hedge lifecycle updates to investor and operations channels.",
            "channels": ("investor-briefings", "ops-war-room"),
        },
        "weight": 0.4,
    },
)
