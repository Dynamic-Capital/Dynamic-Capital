...
function pick(name) {
  return lines.find((x) => x.__name === name) || {};
}
function yn(v) {
  return v ? "yes" : "no";
}
...
function summarizeLogs(j) {
  ...
  const tops = txt.split("\n").filter((s) =>
    /error|fatal|missing|Unauthorized|Secret|Internal/.test(s)
  ).slice(0, 3);
  return `401=${c401} 403=${c403} 500=${c500}${
    tops.length ? `\n  - ${tops.join("\n  - ")}` : ""
  }`;
}

const okAll = ver.bot_ok && ver.mini_ok &&
...
console.log([
  ...
  `- TELEGRAM_WEBHOOK_SECRET (env or DB): ${
    yn(sec.TELEGRAM_WEBHOOK_SECRET || sec.DB_WEBHOOK_SECRET)
  }`,
  ...
  `- self-heal actions: ${
    sync.actions?.length ? JSON.stringify(sync.actions) : "none"
  }`,
  "",
  "OPS HEALTH",
  `- ops-health ok: ${yn(ops.ok)}  db:${yn(ops.db)} secrets_ok:${
    yn(ops.secrets_ok)
  }`,
  "",
  "ADMIN PROBE",
  ...
  okAll
    ? ""
    : "Next steps: resolve mismatches via sync-audit fix; ensure ops-health ok; address 500s seen in logs; verify admin header; redeploy failing functions.",
].join("\n"));
