...
banner(
  "Dynamic Build Suite · Landing Snapshot",
  "Refreshing marketing exports with branded defaults.",
);

if (defaultedKeys.length > 0) {
  note("Applied fallback branding environment variables for this run.", {
...
async function main() {
  step("Preparing backup of current `_static/` export...");
  const hadBackup = await backupStaticDir();

  if (hadBackup) {
    success("Stored a backup of the existing `_static/` snapshot.");
  } else {
    note(
      "No previous `_static/` snapshot detected; continuing without backup.",
    );
  }

  divider();
  step("Refreshing landing snapshot from web workspace...");
  ...
  if (status !== 0) {
    warn(
      "Copy-only snapshot refresh failed. Attempting full rebuild via Next.js…",
    );
  ...
  if (status === 0 && snapshotPresent) {
    await rm(backupDir, { recursive: true, force: true });
    success("Landing snapshot refreshed from Next.js build.");
    celebrate("Landing snapshot build completed without issues.");
    return;
  }

  warn(
    "Unable to refresh landing snapshot from the Next.js app. Preserving the last good export.",
  );

  if (hadBackup && (await restoreBackup())) {
    info("Restored previous `_static/` snapshot from backup.");
  } else {
    await writeFallbackHtml();
    info("Generated a minimal placeholder snapshot so the build can continue.");
  }
...
main().catch((err) => {
  logError("Unexpected error while building landing snapshot.", {
