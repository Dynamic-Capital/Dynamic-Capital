...
(async () => {
  ...
  if (missingEnv.length > 0) {
    ...
    try {
      ...
      const missing = Array.from(supabaseRequired).filter((key) => !present.has(key)).sort();
      matrix.push({
        provider: 'supabase',
  ...
  for (const provider of Array.from(providerMap.keys()).filter((name) => name !== 'supabase')) {
    matrix.push({
      provider,
      status: 'pending',
      note:
  ...
  for (const entry of matrix) {
    const icon =
      entry.status === 'ok' ? '✅' : entry.status === 'missing' ? '❌' : entry.status === 'pending' ? '⏳' : '⚠️';
