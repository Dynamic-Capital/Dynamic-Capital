...
async function supabaseReport(required: Set<string>): Promise<ProviderReport> {
  ...
  if (!process.env.SUPABASE_PROJECT_REF) missingEnv.push('SUPABASE_PROJECT_REF');
  ...
  try {
    ...
    const missingKeys = Array.from(required).filter((key) => !present.has(key)).sort();
...
(async () => {
  ...
  const icon = supabaseSummary.status === 'ok' ? '✅' : supabaseSummary.status === 'missing' ? '❌' : '⚠️';
  console.log(
  ...
  for (const provider of Array.from(providerMap.keys()).filter((p) => p !== 'supabase')) {
    const total = providerMap.get(provider)?.size ?? 0;
    const badge = total === 0 ? '—' : 'ℹ️';
    console.log(`${badge} ${provider}: ${total} keys tracked (sync via env:sync)`);
  }
  ...
  for (const check of appChecks) {
    ...
    console.log(`   Public missing: ${missingPublic === 0 ? 'none' : check.missing.public.join(', ')}`);
    console.log(`   Server missing: ${missingServer === 0 ? 'none' : check.missing.server.join(', ')}`);
  }

  if (supabaseSummary.status === 'ok' && appChecks.every((check) => check.missing.public.length === 0 && check.missing.server.length === 0)) {
    console.log('\nENV OK');
  } else {
