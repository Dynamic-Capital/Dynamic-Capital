...
async function validateSupabase(required: Set<string>): Promise<ProviderSummary> {
  ...
  if (missingEnv.length > 0) {
    return {
      ...
      present: [],
      status: 'error',
      message: `Missing local credentials to query Supabase: ${missingEnv.join(', ')}`,
    };
  }
  ...
  try {
    ...
    const missing = Array.from(required).filter((key) => !present.has(key)).sort();
    const satisfied = Array.from(required).filter((key) => present.has(key)).sort();
...
function printProviderSummary(summary: ProviderSummary): void {
  const icon = summary.status === 'ok' ? '✅' : summary.status === 'missing' ? '❌' : '⚠️';
  ...
  if (summary.present.length > 0) {
    console.log(`   Present (${summary.present.length}): ${summary.present.join(', ')}`);
  }
...
(async () => {
  ...
  const otherProviders = Array.from(providerMap.keys()).filter((provider) => provider !== 'supabase');
  if (otherProviders.length > 0) {
    console.log('\nℹ️  Additional providers configured in env.map.json:', otherProviders.join(', '));
    console.log('    Run `npm run env:sync` once Supabase secrets are up to date to propagate values.');
  }

  const appResults = await loadAppModules();
  for (const result of appResults) {
    const hasMissing = result.missing.public.length > 0 || result.missing.server.length > 0;
  ...
  const missingProviderKeys = supabaseSummary.status !== 'ok' ? supabaseSummary.missing : [];
