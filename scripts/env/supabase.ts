...
function ensureProjectRef(): { projectRef: string; accessToken: string } {
  ...
  if (!projectRef) {
    throw new Error('Missing SUPABASE_PROJECT_REF for Supabase secret operations.');
  }

  if (!accessToken) {
    throw new Error('Missing SUPABASE_ACCESS_TOKEN for Supabase secret operations.');
  }
...
async function runSupabaseCommand(args: string[]) {
  ...
  const { stdout } = await execFileAsync(SUPABASE_CLI, [...args, '--project-ref', projectRef], {
    env: {
      ...process.env,
...
export async function setSecrets(values: Record<string, string>): Promise<void> {
