...
function run(cmd, args, opts = {}) {
  ...
  if (r.status !== 0) {
    const msg = r.stderr?.trim() || r.stdout?.trim() || `${cmd} ${args.join(' ')} failed`;
    throw new Error(msg);
  }
...
(async () => {
  ...
  const baseBranch = process.env.BASE_BRANCH || repoInfo.default_branch || 'main';
  ...
  const fullTree = await gh(`/repos/${owner}/${repo}/git/trees/${treeSha}?recursive=1`);
  ...
  for (const b of blobs) {
    ...
    const hasBadDir = segments.slice(0, -1).some(seg => /[<>:"/\\|?*]|[ .]+$/.test(seg));
    if (bad || hasBadDir) {
      ...
      while (targetSet.has(uniq) || mapping.has(uniq)) {
        ...
        if (extIdx > -1) {
          uniq = `${candidate.slice(0, extIdx)}-renamed-${i}${candidate.slice(extIdx)}`;
        } else {
  ...
  console.log(`Preparing branch ${branchName} with ${offenders.length} renames...`);

  // Build a new tree by applying copies and deletions
  const treeEdits = [];
  for (const o of offenders) {
    treeEdits.push({ path: o.target, mode: o.mode || '100644', type: 'blob', sha: o.sha });
