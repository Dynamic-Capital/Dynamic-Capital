...
const REPO = process.env.GITHUB_REPOSITORY || 'Dynamic-Capital/Dynamic-Capital';
...
async function main(){
  ...
  for(const f of files){
    ...
    try{
      ...
      if(needsSources(text) && !hasSources(text)){
        ...
        violationLabels.add('needs-sources');
        continue;
      }
  ...
  if(violating.length){
    ...
    if(contentTouched) violationLabels.add('content');
    await writeFile('content_guard_labels.txt', Array.from(violationLabels).join('\n'));
