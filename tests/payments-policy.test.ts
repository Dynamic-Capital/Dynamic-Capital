...
test('Bank transfer OCR auto-review', () => {
  ...
  const planPrice = 100;
  const paymentOK: BankPayment = { id: "p2", status: "pending", ocr: { amount: 102, currency: "USD" } };
  const paymentBad: BankPayment = { id: "p3", status: "pending", ocr: { amount: 60, currency: "USD" } };
...
test('Crypto payment auto-completes with confirmations', () => {
  const payment: CryptoPayment = { id: "p4", status: "pending", confirmations: 3 };
