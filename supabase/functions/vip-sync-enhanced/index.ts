    ...
    if (req.method === 'POST') {
      ...
      switch (action) {
        ...
        case 'sync_single_user':
          ...
          return await syncSingleUser(supabase, botToken, telegram_user_id, assign_lifetime);
  ...
  try {
    ...
    for (const channelId of channels) {
      try {
        ...
        if (membersError) {
          console.error(`❌ Error fetching members for ${channelId}:`, membersError);
          continue;
        }

        console.log(`👥 Found ${existingMembers?.length || 0} existing members in ${channelId}`);
        ...
        for (const member of existingMembers || []) {
          try {
            const currentStatus = await getChatMemberStatus(botToken, channelId, member.telegram_user_id);
            const isCurrentlyActive = currentStatus && ['member', 'administrator', 'creator'].includes(currentStatus);
            ...
            if (member.is_active !== isCurrentlyActive) {
              ...
              if (!updateError) {
                channelUpdated++;
                console.log(`✅ Updated ${member.telegram_user_id}: ${member.is_active} → ${isCurrentlyActive}`);
                
                // Recompute VIP status for this user
                await recomputeUserVipStatus(supabase, member.telegram_user_id);
              } else {
                console.error(`❌ Failed to update ${member.telegram_user_id}:`, updateError);
              }
            }
            ...
            console.error(`❌ Error checking member ${member.telegram_user_id}:`, error);
          }
        }
        ...
        console.log(`✅ Channel ${channelId}: ${channelSynced} checked, ${channelUpdated} updated`);
    ...
    await supabase.from('admin_logs').insert({
      ...
      action_description: `Synced ${totalSynced} members across ${channels.length} channels. Updated ${totalUpdated} memberships.`,
      affected_table: 'channel_memberships'
    });

    console.log(`🎉 VIP sync completed: ${totalSynced} checked, ${totalUpdated} updated`);
...
async function assignLifetimeToCurrentMembers(supabase: any, botToken: string) {
  ...
  try {
    ...
    if (!lifetimePlan) {
      ...
      const { data: newPlan, error: createError } = await supabase
        ...
        .insert({
          ...
          duration_months: 999,
          is_lifetime: true,
          features: ['Lifetime VIP Access', 'All Premium Features', 'Priority Support']
        })
        .select()
        .single();
    ...
    for (const member of activeMembers) {
      try {
        ...
        if (existingSub) {
          ...
          if (updateError) {
            console.error(`❌ Failed to update subscription for ${member.telegram_user_id}:`, updateError);
            results.push({ telegram_user_id: member.telegram_user_id, status: 'error', error: updateError.message });
          } else {
            assignedCount++;
            console.log(`✅ Updated existing subscription for ${member.telegram_user_id}`);
            results.push({ telegram_user_id: member.telegram_user_id, status: 'updated' });
          }
        } else {
          ...
          if (insertError) {
            console.error(`❌ Failed to create subscription for ${member.telegram_user_id}:`, insertError);
            results.push({ telegram_user_id: member.telegram_user_id, status: 'error', error: insertError.message });
          } else {
            assignedCount++;
            console.log(`✅ Created lifetime subscription for ${member.telegram_user_id}`);
            results.push({ telegram_user_id: member.telegram_user_id, status: 'created' });
          }
        }
        ...
        results.push({ telegram_user_id: member.telegram_user_id, status: 'error', error: error.message });
      }
    }
    ...
    await supabase.from('admin_logs').insert({
      ...
      action_description: `Assigned lifetime membership to ${assignedCount} current VIP members`,
      affected_table: 'user_subscriptions'
    });

    console.log(`🎉 Lifetime assignment completed: ${assignedCount} members assigned`);
...
async function syncSingleUser(supabase: any, botToken: string, telegramUserId: string, assignLifetime = false) {
  ...
  try {
    ...
    for (const channelId of channels) {
      const status = await getChatMemberStatus(botToken, channelId, telegramUserId);
      const isActive = status && ['member', 'administrator', 'creator'].includes(status);
    ...
    if (assignLifetime && isVipInAnyChannel) {
      const lifetimeResult = await assignLifetimeToCurrentMembers(supabase, botToken);
      return ok({
...
async function recomputeUserVipStatus(supabase: any, telegramUserId: string) {
  try {
    ...
    console.error(`❌ Error recomputing VIP status for ${telegramUserId}:`, error);
    return false;
  }
}