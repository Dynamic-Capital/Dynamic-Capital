  ...
  try {
    ...
    promos?.forEach(
      (
        ...
        const discount = promo.discount_type === "percentage"
          ...
          : `$${promo.discount_value}`;
        const autoBadge = promo.auto_created ? "🤖 Auto" : "✍️ Manual";
        const validUntil = promo.valid_until
          ? new Date(promo.valid_until).toLocaleDateString()
          : "N/A";
        promoMessage += `${
          index + 1
        }. ${status} **${promo.code}** (${autoBadge})\n`;
        promoMessage += `   💰 ${discount} ${promo.discount_type}\n`;
        promoMessage += `   📅 Valid until: ${validUntil}\n`;
        if (promo.generated_via) {
          promoMessage += `   ⚙️ Trigger: ${promo.generated_via}\n`;
        }
    ...
    const promoKeyboard = {
      inline_keyboard: [
        ...
        [
          { text: "🤖 Auto Generate", callback_data: "auto_generate_promo" },
        ],
        [
...
export async function handleAutoGeneratePromo(
  chatId: number,
  _userId: string,
): Promise<void> {
  try {
    ...
    if (error) {
      console.error("Error invoking promo-auto-generate:", error);
      await sendMessage(
        chatId,
        "❌ Failed to trigger auto promo generator. Please try again later.",
      );
      return;
    }
    ...
    if (!result) {
      await sendMessage(
        chatId,
        "❌ Promo generator returned an empty response.",
      );
      return;
    }

    if (result.ok === false) {
      const reason = result.reason || "No trigger";
      const extra = result.message ? `\n${result.message}` : "";
      await sendMessage(
        chatId,
        `ℹ️ Auto generator did not create a promo.\nReason: *${reason}*${extra}`,
      );
      return;
    }
    ...
    if (result.code) {
      const discountText = result.discount
        ? result.discount.discountType === "percentage"
          ...
          : `$${result.discount.discountValue}`
        : "(unspecified)";
      const planName = result.plan?.name ?? "VIP package";
      ...
      await sendMessage(
        chatId,
        `✅ *Auto promo generated!*\n\n• Code: *${result.code}*\n• Discount: *${discountText}*\n• Target: *${planName}*\n• Valid until: *${validUntil}*`,
      );
    } else {
      await sendMessage(
        chatId,
        "✅ Promo generator executed with no new code.",
      );
    }
  } catch (err) {
    console.error("Exception in handleAutoGeneratePromo:", err);
    await sendMessage(
      chatId,
      "❌ Unexpected error running auto promo generator.",
    );
  }
}
...
const exportedAdminHandlers = {
  handleAdminDashboard,
  handleTableManagement,
  handleKvConfigManagement,
  handleAbuseBansManagement,
  ...
  handleRemovePlanFeature,
  handleReplacePlanFeatures,
  handlePlanEditInput,
  handleCreateVipPlan,
  handleDeleteVipPlan,
  ...
  handleAutoGeneratePromo,
  handleDailyAnalyticsManagement,
  handleUserSessionsManagement,
  handlePaymentsManagement,
  handleBroadcastMessagesManagement,
  handleBankAccountsManagement,
  handleTableStatsOverview,
  handleUserInteractionsManagement,
  handleChannelMembershipsManagement,
  handleMediaFilesManagement,
  handleAdminLogsManagement,
  ...
  handlePublishFlagsRequest,
  handlePublishFlagsConfirm,
  handleRollbackFlagsRequest,
  handleRollbackFlagsConfirm,
};

export default exportedAdminHandlers;
