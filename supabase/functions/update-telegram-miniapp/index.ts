...
async function callTelegramAPI(botToken: string, method: string, body?: any): Promise<TelegramResponse> {
...
export const handler = registerHandler(async (req) => {
  ...
  try {
    ...
    if (!miniAppUrl) {
      ...
      miniAppUrl = autoUrl ? (autoUrl.endsWith("/") ? autoUrl : `${autoUrl}/`) : null;
    }
    ...
    const currentMenuResponse = await callTelegramAPI(botToken, 'getChatMenuButton');
    ...
    if (currentMenuResponse.ok && currentMenuResponse.result?.web_app?.url) {
      ...
      console.log('Current URL:', currentUrl, 'New URL:', miniAppUrl, 'Needs update:', needsUpdate);
    }
    ...
    if (needsUpdate) {
      ...
      if (!menuButtonResult.ok) {
        throw new Error(`Failed to update menu button: ${menuButtonResult.description}`);
      }
    }
    ...
    if (webhookUrl && webhookSecret) {
      const currentWebhookResponse = await callTelegramAPI(botToken, 'getWebhookInfo');
      
      if (currentWebhookResponse.ok) {
        const currentWebhookUrl = currentWebhookResponse.result?.url;
        
        if (currentWebhookUrl !== webhookUrl) {
          console.log('Updating webhook URL from', currentWebhookUrl, 'to', webhookUrl);
    ...
    return new Response(JSON.stringify({
      success: true,
      message: needsUpdate ? 'Mini App URL updated successfully' : 'Mini App URL already up to date',
      botInfo: {
