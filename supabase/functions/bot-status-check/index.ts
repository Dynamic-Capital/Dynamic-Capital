  ...
  try {
        ...
        data: botInfo.ok
          ? {
            ...
            can_read_all_group_messages:
              botInfo.result.can_read_all_group_messages,
    ...
    if (!status.bot_info.success) {
      status.recommendations.push(
        "❌ Bot token is invalid. Check TELEGRAM_BOT_TOKEN in Supabase secrets",
      );
    }

    if (!status.webhook_info.data?.url) {
      status.recommendations.push(
        "⚠️ No webhook URL set. Use setup-webhook-helper to configure",
      );
    }

    if (!status.secrets.env_secret_exists) {
      status.recommendations.push(
        "❌ TELEGRAM_WEBHOOK_SECRET not set in Supabase secrets",
      );
    }

    if (
      !status.secrets.secrets_match && status.secrets.db_secret_exists &&
      status.secrets.env_secret_exists
    ) {
      status.recommendations.push(
        "⚠️ Webhook secrets don't match between database and environment",
      );
    }

    if (status.webhook_info.data?.pending_update_count > 0) {
      status.recommendations.push(
        `⚠️ ${status.webhook_info.data.pending_update_count} pending updates. Consider clearing them`,
      );
    }

    if (status.webhook_info.data?.last_error_message) {
      status.recommendations.push(
        `❌ Last webhook error: ${status.webhook_info.data.last_error_message}`,
      );
    }
    ...
    const response = oops("Failed to check bot status", {
      error: error.message,
    });
    return new Response(response.body, {
      status: response.status,
