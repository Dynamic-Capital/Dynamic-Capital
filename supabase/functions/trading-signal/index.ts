import { normalizeTradingViewAlert, type TradingViewAlertPayload } from '../../../apps/web/integrations/tradingview/alert.ts';
import { bad, jsonResponse, methodNotAllowed } from '../_shared/http.ts';
import { registerHandler } from '../_shared/serve.ts';
import { createClient } from '../_shared/client.ts';
import { ensureFeatureRegistered, isFeatureEnabled } from '../_shared/features.ts';
...
export const handler = registerHandler(async (req) => {
  ...
  try {
    ...
    return jsonResponse({ ok: false, error: (error instanceof Error ? error.message : 'Invalid payload') }, { status: 400 }, req);
  }
  ...
  if (error) {
    ...
    return jsonResponse({ ok: false, error: 'Failed to persist signal' }, { status: 500 }, req);
  }

  if (!featureEnabled) {
    console.warn('[tradingview] feature disabled, skipping downstream dispatch');
    return jsonResponse({ status: 'queued', alertId: normalized.alertId, enabled: false }, { status: 202 }, req);
  }

  return jsonResponse({ status: 'ok', alertId: normalized.alertId }, { status: 200 }, req);
});
