import { buildCallbackAck, getBinancePayCredentials, readCallbackHeaders, verifyCallbackSignature } from '../../../apps/web/integrations/binance/client.ts';
...
export const handler = registerHandler(async (req) => {
  ...
  await ensureFeatureRegistered(FEATURE_KEY, 'Enable Binance Pay callback ingestion', false, {
    gateway: 'binance-pay',
  });
  ...
  if (!signatureValid) {
    ...
    return jsonResponse({ ok: false, error: 'Signature verification failed' }, { status: 401 }, req);
  }
