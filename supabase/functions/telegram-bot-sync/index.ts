...
interface TelegramBotSyncRequest {
  action: 'sync_user' | 'sync_payment' | 'sync_subscription' | 'get_user_status';
...
export const handler = registerHandler(async (req) => {
  ...
  try {
    ...
    const { action, telegram_user_id, data }: TelegramBotSyncRequest = await req.json();
    ...
    console.log(`Processing telegram bot sync: ${action} for user ${telegram_user_id}`);
...
async function syncPayment(supabase: any, telegram_user_id: string, paymentData: any) {
  try {
    // Get user ID
...
async function syncSubscription(supabase: any, telegram_user_id: string, subscriptionData: any) {
  try {
...
async function getUserStatus(supabase: any, telegram_user_id: string) {
  try {
    ...
    const status = {
      ...
      subscription_status: userData.user_subscriptions?.[0]?.payment_status || 'none',
