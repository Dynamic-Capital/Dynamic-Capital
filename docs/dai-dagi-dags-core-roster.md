...
> _Note:_ The “Watcher” role in the infrastructure reference is the preferred
> term for the “looker” persona mentioned in field operations
> briefings.【F:docs/model-intelligence-infrastructure-reference.md†L94-L108】

**Knowledge cadence.** DCM8 memory exports and DCM10 validation bundles flow
into DCR3 and DCR1 ledgers, while DCM9 context packs keep DCH4 and DCH7
briefings aligned—mirroring the protocol guidance that channels knowledge corpus
updates through `dynamic_memory` builders and documented briefs in
`dynamic_library`.【F:docs/dynamic-capital-model-context-protocol.md†L36-L78】

**Knowledge surfaces.**

- **Knowledge sharing:** Assistants and Helpers formalise ingestion and modeling
  briefs in `dynamic_library`, maintaining conversational and procedural
  packages for downstream teams.【F:docs/dynamic_inventory.md†L41-L228】
- **Memory retention:** Keepers orchestrate `dynamic_memory` and
  `dynamic_memory_reconsolidation` vaults so Adaptive Learning (DCM6) and Memory
  Management (DCM8) share reconciled state with AGI and AGS
  domains.【F:docs/dynamic_inventory.md†L108-L228】
- **Storage replication:** Bot-led crawlers and Keepers replicate large payloads
  into `dynamic_blob_storage` and `dynamic_storage` queues to guarantee
  artefacts stay available for validation and audit
  drills.【F:docs/dynamic_inventory.md†L41-L117】【F:docs/dynamic-capital-model-context-protocol.md†L52-L88】
...
- **Knowledge sharing:** Assistants and Helpers syndicate prompts,
  retrospectives, and synthesis digests via `dynamic_library`, giving every
  capability lane shared visibility into language, planning, and social
  playbooks.【F:docs/dynamic_inventory.md†L212-L228】【F:docs/dynamic-capital-model-context-protocol.md†L36-L78】
- **Memory retention:** Keepers manage `dynamic_memory` and
  `dynamic_memory_reconsolidation` footprints dedicated to AGI retrospectives so
  Self-Reflection (DCH8) and Cross-Domain Transfer (DCH9) can replay lessons
  without data loss.【F:docs/dynamic_inventory.md†L108-L228】
- **Storage replication:** Developers and Builders replicate larger scenario
  models and rollout manifests into `dynamic_storage` to keep downstream
  governance and AI meshes aligned on the same
  artefacts.【F:docs/dynamic_inventory.md†L41-L117】【F:docs/dynamic-capital-model-context-protocol.md†L36-L88】
