...
```json
{
  ...
  "records": {
    ...
    "metadata_fallback": "https://dynamic-capital.ondigitalocean.app/jetton-metadata.json",
...
The fallback keeps the DigitalOcean reverse proxy in lockstep with the on-chain
metadata so integrators can resolve the JSON even when `.ton` gateways are
temporarily offline.
