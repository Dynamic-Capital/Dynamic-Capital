...
| `dex_router` (`dct_app_config`)        | Router endpoint that swaps TON → DCT inside [`dct-auto-invest`](../supabase/functions/dct-auto-invest/index.ts).                  | `EQAyD7O8CvVdR8AEJcr96fHI1ifFq21S8QMt1czi5IfJPyfA` | Coordinate with Liquidity Ops; update the DNS resolver and Supabase row atomically.            |
