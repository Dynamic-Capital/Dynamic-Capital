...
real-time APIs (for example, the OpenAI Realtime API). Use `npm:ws` in `noServer`
mode so you can handle the upgrade yourself and fan out events between the
browser client and the upstream service.
