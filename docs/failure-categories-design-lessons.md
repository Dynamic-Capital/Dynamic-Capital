...
3. **Run the production build:** Execute `npm run build` immediately after the verify sweep and attach the logs to the same record so release readiness is validated alongside telemetry freshness.
4. **Cross-check telemetry:** Compare verify output against the safeguard engine scorecard to confirm dashboard freshness, playbook coverage, and drill cadence metrics.
5. **Update the artifact trail:** Attach the verify summary and build logs to the mentorship case study archive and Oracle scoring notes so future cohorts inherit the evidence.
