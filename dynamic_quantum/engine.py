...
class DynamicQuantumEngine:
    ...
    def __init__(self, *, window: int = 180, equilibrium_target: float = 0.72) -> None:
        ...
        self._pulses: Deque[QuantumPulse] = deque(maxlen=window)
    ...
    def synthesize_frame(self, environment: QuantumEnvironment | None = None) -> QuantumResonanceFrame:
        ...
        pulses = self.pulses
        mean_coherence = fmean(p.coherence for p in pulses)
        mean_entanglement = fmean(p.entanglement for p in pulses)
        mean_flux = fmean(p.flux for p in pulses)
        mean_phase_variance = fmean(p.phase_variance for p in pulses)
