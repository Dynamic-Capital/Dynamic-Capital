...
class DatabaseHandler:
    ...
    async def async_get_trades_by_mt5_tickets(self, mt5_tickets: List[str]) -> Dict[str, Dict[str, Any]]:
        """Fetch multiple trades mapped by MT5 ticket for efficient lookups."""

        unique_tickets = [ticket for ticket in {t for t in mt5_tickets if t}]
        ...
        def _get_trades():
            with self.get_db() as db:
                try:
                    rows = (
                        db.query(Trade)
                        .filter(Trade.mt5_ticket.in_(unique_tickets))
                        .all()
                    )

                    result: Dict[str, Dict[str, Any]] = {}
                    for trade in rows:
                        result[str(trade.mt5_ticket)] = {
                            "trade_id": trade.trade_id,
                            "position_id": trade.position_id,
                            "mt5_ticket": trade.mt5_ticket,
                            "instrument": trade.instrument,
                            ...
                            "status": trade.status,
                            "is_closed": trade.is_closed,
                            "trailing_stop_pips": float(trade.trailing_stop_pips)
                            if trade.trailing_stop_pips is not None
                            else None,
                    ...
                    logger.error(f"Error in async get trades by MT5 tickets: {e}")
                    logger.error(traceback.format_exc())
                    raise

        return await self.loop.run_in_executor(None, _get_trades)
