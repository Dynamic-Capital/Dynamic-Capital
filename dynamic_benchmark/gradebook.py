...
class GradeThresholds:
    """Metric thresholds required to achieve a grade band."""
...
_RULES: tuple[_GradeRule, ...] = (
    _GradeRule(
        ...
        failed_checks_max=0,
        rationale=(
            "Knowledge base leadership – catalogue depth, precision, and telemetry "
            "refreshes deliver elite, industry benchmark execution."
        ),
        remediation=(
            "Codify elite practices, expand mentorship rotations, and capture "
            "reinforcement datasets to preserve institutional mastery."
        ),
    ),
    _GradeRule(
        band="AA+ / AA",
        ...
        failed_checks_max=0,
        rationale=(
            "Premier coverage with precision telemetry; only incremental refinements "
            "separate the estate from elite stewardship."
        ),
        remediation=(
            "Drive continuous tuning, publish exemplars, and sponsor cross-domain "
            "audits to sustain momentum toward AAA stature."
        ),
    ),
    _GradeRule(
        band="A / A-",
...
def grade_ranking() -> tuple[str, ...]:
    """Return grade letters ordered from highest to lowest."""

    return tuple(rule.letter for rule in _RULES) + ("D",)


def grade_thresholds_for(letter: str) -> GradeThresholds:
    """Lookup the metric thresholds associated with a grade letter."""

    for rule in _RULES:
        if rule.letter == letter:
            return GradeThresholds(
                coverage_ratio=rule.coverage_min,
                accuracy_ratio=rule.accuracy_min,
                telemetry_staleness_hours=rule.staleness_max,
                failed_health_checks=rule.failed_checks_max,
            )
    raise KeyError(f"Unknown grade letter: {letter}")
