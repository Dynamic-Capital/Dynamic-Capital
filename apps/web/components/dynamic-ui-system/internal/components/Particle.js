...
import { Flex } from ".";
const Particle = React.forwardRef(
  (
    {
      density = 100,
    ...
    useEffect(() => {
      ...
      const createParticle = () => {
        ...
        particleEl.style.transition =
          "transform 0.4s ease-out, opacity 0.6s ease-out";
      ...
      const updateParticles = () => {
        particles.forEach((particleEl, index) => {
          ...
          particleEl.style.left = `${
            currentX + (targetX - currentX) * movementSpeed
          }%`;
          particleEl.style.top = `${
