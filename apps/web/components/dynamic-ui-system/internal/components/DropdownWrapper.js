...
const DropdownWrapper = forwardRef(({
  ...
  const handleClickOutside = useCallback((event) => {
    ...
    const isClickInDropdown = dropdownRef.current &&
      dropdownRef.current.contains(target);
    const isClickInWrapper = wrapperRef.current &&
    ...
    if (
      ...
      if (isClickInDropdown || isClickOnDropdownPortal) {
        ...
        const allPortals = document.querySelectorAll(
  ...
  const handleFocusOut = useCallback((event) => {
    ...
    const isFocusInDropdown = dropdownRef.current &&
      dropdownRef.current.contains(event.relatedTarget);
    const isFocusInWrapper = wrapperRef.current &&
