...
export const useArrowNavigation = ({
  ...
  const findNextEnabledItem = useCallback((currentIndex, direction) => {
  ...
  const findEnabledItemFromIndex = useCallback((startIndex) => {
    if (!containerRef?.current) {
      return startIndex;
    }
    const items = Array.from(
      ...
      case "ArrowUp":
        ...
        if (layout === "column") {
          ...
          if (nextIndex >= 0) {
            ...
            newIndex = findEnabledItemFromIndex(
  ...
  const applyHighlightedState = useCallback(() => {
    if (!containerRef?.current || disableHighlighting) {
      return;
    }
    const items = Array.from(
      containerRef.current.querySelectorAll(itemSelector),
