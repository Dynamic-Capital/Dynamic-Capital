...
import { Check, CreditCard, Sparkles, Star, TrendingUp } from "@/lib/lucide";
...
export default function PlanSection() {
    ...
    const fetchData = async () => {
      try {
        ...
        const contents: ContentItem[] = (contentResult.data as any)?.contents ||
          [];
        const popularContent = contents.find((c) =>
          c.content_key === "popular_plan_id"
        );
  ...
  const validatePromoCode = async () => {
    ...
    try {
      ...
      const telegramUserId = isInTelegram
  ...
  const handleSelectPlan = (planId: string) => {
    ...
    const selectionCounts = JSON.parse(
      localStorage.getItem("plan_selection_counts") || "{}",
    );
                      ...
                      >
                        ...
                        >
                          ...
                          {promoValidation?.valid &&
                            ...
                            ? (currency === "MVR"
                              ? Math.round(
                                promoValidation.final_amount * exchangeRate,
                              )
                              : promoValidation.final_amount)
                        ...
                        <AnimatePresence>
                          {promoValidation?.valid &&
                            promoValidation.final_amount !== plan.price && (
                            <motion.div
