...
const registerImpl: () => Promise<void> = (() => {
  ...
  function resolveSDKMetricsExports(
    ...
    exports?: SDKMetricsExports;
    diagnostics: Record<SDKMetricsExportKey, boolean>;
  } {
    const diagnostics = SDK_METRICS_EXPORT_KEYS.reduce(
      (acc, key) => ({ ...acc, [key]: false }),
      {} as Record<SDKMetricsExportKey, boolean>,
    );

    const resolvedEntries: Partial<Record<SDKMetricsExportKey, unknown>> = {};

    for (const key of SDK_METRICS_EXPORT_KEYS) {
      const guard = SDK_METRICS_EXPORT_GUARDS[key];
      const candidate = (module[key] ?? module.default?.[key]) as unknown;

      if (guard(candidate)) {
        diagnostics[key] = true;
        resolvedEntries[key] = candidate;
      }
    }
