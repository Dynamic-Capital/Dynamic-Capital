...
function resolveDefaultDomain() {
  const fallback = brand.identity.domain;
...
<svg width="${OG_WIDTH}" height="${OG_HEIGHT}" viewBox="0 0 ${OG_WIDTH} ${OG_HEIGHT}" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="172" y="188" text-anchor="middle" font-family="'Inter','Segoe UI',sans-serif" font-weight="700" font-size="48" fill="#020617">${brand.identity.abbreviation}</text>
  ...
  <text x="120" y="540" font-family="'Inter','Segoe UI',sans-serif" font-weight="600" font-size="30" fill="#F8FAFC">${brand.identity.name}</text>
