...
const AMBIGUOUS_RESOURCE_ERROR = Object.freeze({
  status: "error",
  message: "Multiple resource slugs provided. Specify a single resource.",
  availableResources: RESOURCE_SLUGS,
});

const UNKNOWN_RESOURCE_ERROR = Object.freeze({
  status: "error",
  message: "Unknown dynamic REST resource",
  availableResources: RESOURCE_SLUGS,
});

const ambiguousResourceResponse = (req: Request) =>
  jsonResponse(AMBIGUOUS_RESOURCE_ERROR, { status: 400 }, req);

const unknownResourceResponse = (req: Request) =>
  jsonResponse(UNKNOWN_RESOURCE_ERROR, { status: 404 }, req);
...
export async function GET(req: Request, context: RouteHandlerContext) {
  ...
  const routeName = resolution.type === "ok"
