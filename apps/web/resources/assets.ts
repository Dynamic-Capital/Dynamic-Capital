...
const DYNAMIC_CAPITAL_BUCKET = "dynamic-capital";
...
function buildBucketBase(): string {
  try {
    const bucketUrl = new URL(
      `/storage/v1/object/public/${DYNAMIC_CAPITAL_BUCKET}/`,
      SUPABASE_URL,
    );
    return bucketUrl.toString();
  } catch {
    return `${DEFAULT_SUPABASE_URL}/storage/v1/object/public/${DYNAMIC_CAPITAL_BUCKET}/`;
  }
}
...
export const dynamicCapitalBucketBaseUrl = SUPABASE_BUCKET_BASE;
export const dynamicCapitalBucketName = DYNAMIC_CAPITAL_BUCKET;
