...
describe("dex-screener service", () => {
  ...
  it("builds a dex screener response envelope", async () => {
    ...
    expect(response.metadata.cache.ttlSeconds).toBe(
      DEX_SCREENER_CACHE_TTL_SECONDS,
    );
    expect(response.metadata.cache.control).toBe(
      DEX_SCREENER_CACHE_CONTROL_HEADER,
    );
    expect(response.metadata.limits.maxResults).toBeGreaterThan(0);
    expect(response.metadata.endpoints).toEqual(DEX_SCREENER_API_ENDPOINTS);
