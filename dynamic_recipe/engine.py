...
class DynamicRecipeEngine:
    ...
    def plan(self, context: RecipeContext, *, limit: int = 3) -> RecipePlan:
        ...
        evaluations = [
            ...
            if (evaluation := self._evaluate_recipe(recipe, context)) is not None
        ]
        if not evaluations:
    ...
    def _evaluate_recipe(self, recipe: RecipeProfile, context: RecipeContext) -> _RecipeEvaluation | None:
        recipe_ingredients = {ingredient.name for ingredient in recipe.ingredients}
        if recipe_ingredients & set(context.disliked_ingredients):
            return None
        missing_dietary = [style for style in context.dietary_styles if style not in recipe.tags]
        if missing_dietary:
            return None
